...
Tài liệu này được tổng hợp từ bộ nguồn PCForge cho repository **SBA301**. Đường dẫn trong các lệnh bên dưới tính từ thư mục gốc repository; xem [cấu trúc dự án](architecture/project-structure.md).
...
├── database/
│   ├── bootstrap/bootstrap.sql     # Tạo database pcforge
│   ├── exports/PCForge_MySQL_Full.txt
│   ├── examples/promotion_examples.sql
│   └── tests/
│       ├── package.json
│       ├── validate-schema.cjs
│       └── smoke.sql
└── docs/database.md                # Tài liệu này
```

Tải hoặc xem trực tiếp: [bootstrap](../database/bootstrap/bootstrap.sql), [schema V1](../backend/src/main/resources/db/migration/V1__schema.sql), [dữ liệu nền V2](../backend/src/main/resources/db/migration/V2__reference_data.sql), [SQL tổng hợp](../database/exports/PCForge_MySQL_Full.txt), [ví dụ khuyến mãi](../database/examples/promotion_examples.sql) và [smoke test](../database/tests/smoke.sql).
...
```sql
SOURCE database/bootstrap/bootstrap.sql;
USE pcforge;
SOURCE backend/src/main/resources/db/migration/V1__schema.sql;
SOURCE backend/src/main/resources/db/migration/V2__reference_data.sql;
...
Đây là phương án thay cho import V1/V2 thủ công. Chạy bootstrap trước; hai migration đã nằm trong backend tại:

```text
backend/src/main/resources/db/migration/V1__schema.sql
backend/src/main/resources/db/migration/V2__reference_data.sql
```
...
Chạy lại kiểm tra tĩnh từ thư mục `database/tests` của repository:
...
```sql
SOURCE database/tests/smoke.sql;
```
